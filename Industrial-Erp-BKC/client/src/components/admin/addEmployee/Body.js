...
import { ADD_EMPLOYEE, SET_ERRORS } from "../../../redux/actionTypes";
...
const Body = () => {
  ...
  useEffect(() => {
    if (store.errors || store.admin.employeeAdded) {
      setLoading(false);
      if (store.admin.employeeAdded) {
        setValue({
          name: "",
          dob: "",
          email: "",
        ...
        dispatch({ type: ADD_EMPLOYEE, payload: false });
      }
    } else {
      setLoading(true);
    }
  }, [store.errors, store.admin.employeeAdded]);
