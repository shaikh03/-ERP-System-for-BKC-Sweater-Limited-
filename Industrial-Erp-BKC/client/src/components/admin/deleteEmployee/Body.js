...
import { DELETE_EMPLOYEE, SET_ERRORS } from "../../../redux/actionTypes";
...
const Body = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setError({});
    dispatch(getEmployee(value));
  };
  const employees = useSelector((state) => state.admin.employees.result);

  const dltEmployee = (e) => {
    setError({});
    setLoading(true);
    dispatch(deleteEmployee(checkedValue));
  };

  useEffect(() => {
    if (store.admin.employeeDeleted) {
      setLoading(false);
      setValue({ department: "" });
      dispatch({ type: DELETE_EMPLOYEE, payload: false });
      setSearch(false);
    }
  }, [store.admin.employeeDeleted]);
        ...
        <div className=" mr-10 bg-white grid grid-cols-4 rounded-xl pt-6 pl-6 h-[29.5rem]">
          ...
          <div className="col-span-3 mr-6">
            <div className={classes.loadingAndError}>
              ...
              {(error.noEmployeeError || error.backendError) && (
